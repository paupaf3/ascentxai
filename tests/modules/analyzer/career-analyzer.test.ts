...
const mockedExtractLinkedIn = extractLinkedInProfile as ReturnType<
    typeof vi.fn
>;
...
describe("analyze", () => {
    ...
    it("throws when the agent returns empty text", async () => {
        ...
        await expect(
            analyze("/tmp/resume.pdf", "testuser", TARGET_GOAL, "")
        ).rejects.toThrow("Career analysis agent returned an empty response.");
    });

    it("propagates errors from extractCandidateProfile", async () => {
        mockedExtract.mockRejectedValueOnce(
            new Error("NIM_API_KEY is not set")
        );
        await expect(
            analyze("/tmp/resume.pdf", "testuser", TARGET_GOAL, "")
    ...
    it("propagates errors from fetchProfile", async () => {
        ...
        await expect(
            analyze("/tmp/resume.pdf", "testuser", TARGET_GOAL, "")
        ).rejects.toThrow('GitHub user "testuser" not found');
    });

    it("propagates errors from extractLinkedInProfile", async () => {
        mockedExtractLinkedIn.mockRejectedValueOnce(
            new Error("PDF has no extractable text")
        );
        await expect(
            analyze(
                "/tmp/resume.pdf",
