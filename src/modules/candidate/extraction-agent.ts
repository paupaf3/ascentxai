...
export const candidateExtractionAgent = new Agent({
    ...
    instructions: [
        ...
        '- For each role, fill startDate and endDate as YYYY or YYYY-MM. Use the literal string "present" for current roles.',
        '- Compute yearsInRole for each role as a number (fractional allowed, e.g. 1.5) from its own start/end dates.',
        '- Compute totalYearsOfExperience as the sum of NON-OVERLAPPING role periods. If two roles overlap, count the overlap only once. Do not simply add yearsInRole together.',
